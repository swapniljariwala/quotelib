...
def get_quote(symbol,exchange = 'NSE'):
    ...
    #opener = urllib2.build_opener(HTTPHandler(),HTTPSHandler(),HTTPErrorProcessor(),HTTPCookieProcessor(cj))
    req = requests.Session()
    post_data = symbol +  '|^ALL|^Q|^|$'
    resp = req.post(ICICI_QUOTE_URL,data = post_data)
...
def get_market_depth(symbol,exchange='NSE'):
    ...
    resp = requests.post(ICICI_QUOTE_URL, data = post_data)
